...
class DeepONet(nn.Module):
    ...
    def forward(self, u: torch.Tensor, x: torch.Tensor) -> torch.Tensor:
        ...
        out = torch.vmap(
            torch.vmap(
                lambda bs, ts: torch.sum(bs * ts),
                in_dims=(None, 0)
            ),
            in_dims=(0, None)
        )(branch_val, trunk_val)
        return out
...
class MIONet(nn.Module):
    def __init__(
        ...
        for branch_dim in branch_dim_list:
            self.branch_list.append(MLP(branch_dim[0], branch_dim[1:-1], 
                                        branch_dim[-1], activation))
        self.trunk = MLP(trunk_dim[0], trunk_dim[1:-1], trunk_dim[-1], activation)
    
    def forward(self, func_list: list[torch.Tensor], x: torch.Tensor) -> torch.Tensor:
