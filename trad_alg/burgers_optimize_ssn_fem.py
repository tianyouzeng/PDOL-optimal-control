...
Modified on Thu Dec 19 14:57:14 2024
...
# Change SSN iteration to 6, compared with 20 in the original script,
# to make the runtime more comparable to the DeepONet implementation.
for i in range(6):
    Dy=D*y
    DTp=D.transpose()*p
    ...
    inact=(u+lambda_dual<=b*ee).astype(np.float64)
    InAct=spdiags(inact[:,0], 0, N-1, N-1)
    act=(u+lambda_dual>b*ee).astype(np.float64)
    ...
    righthand2=yd*ee-y+ nu/h**2*A*p+(Y*D*p)/h
...
plt.legend()
if save_plots:
    plt.savefig(f"trad_alg/results/computed_u_ssn_alpha_{alpha}_yd_{yd}_res_{N}.png")
plt.show()
...
plt.legend()
if save_plots:
    plt.savefig(f"trad_alg/results/computed_y_ssn_alpha_{alpha}_yd_{yd}_res_{N}.png")
plt.show()

if save_results:
    np.savez(f"trad_alg/results/computed_u_y_ssn_alpha_{alpha}_yd_{yd}_res_{N}.npz", u=u, y=y)
