...
This script solves the following semilinear parabolic optimal control problem 
    with the FEM-based semismooth Newton method on the spatial-temporal 
    domain [0, 1]^2 x [0, 1]:
...
def R_grad_grad_func(y: npt.NDArray) -> npt.NDArray:
    ''' Second-order Frechet derivative of R(y) '''
    # Note that R''(u) acts on any w1, w2 as a pointwise product
    # For convenience, we return the R''(u) as the function multiplied to w1 * w2
    return (6.0 * y + 1.5)
...
def S_grad_func(
    ...
    if matrix_k_list is None:
        R_grad_S_u = R_grad_func(S_u)
        matrix_k_list = update_matrix_k_list(R_grad_S_u)
...
def G_grad_func(
    ...
    for i in range(resol_time):
        ...
        rhs_n[free_node, 0] = ((matrix_m / tau) @ p_later[free_node,0] 
                               + matrix_m @ (
                                   S_grad_u_w[free_node, n:n+1] 
                                   - R_grad_grad_func(S_u)[free_node, n:n+1] 
                                   * G_u[free_node, n:n+1] * S_grad_u_w[free_node, n:n+1]
                                )[:,0])
        p_now = np.zeros([num_dof, 1])
        p_now[is_bd_node, 0] = boundary_cond_func(node[is_bd_node])
...
def get_ssn_step(
    ...
    phi_u = phi_func(u, yd, alpha, beta, u_a, u_b, G_u)
    ...
    r = (is_inactive_nodes * (p + (1.0 / alpha) * G_grad_func(
            u, p_inactive, yd, S_u, S_grad_u_pi, G_u, matrix_k_list
        )) + is_inactive_nodes * (phi_u + (1.0 / alpha) * G_grad_func(
            u, p_active, yd, S_u, S_grad_u_pa, G_u, matrix_k_list
        )))
    ...
    while r_norm > tol and iter < max_cg_iter:
        ...
        A_q = is_inactive_nodes * (q + (1.0 / alpha) * G_grad_func(
            u, is_inactive_nodes * q, yd, S_u, S_grad_u_q, G_u, matrix_k_list
        ))
...
if __name__ == "__main__":
    ...
    with (multiprocessing.Pool(processes=process_count) if use_multiprocessing 
        ...
        for iter_ssn in range(max_iter_ssn):
            ...
            matrix_k_list = update_matrix_k_list(R_grad_Su)
            G_u = G_func(u, yd, S_u, matrix_k_list)
            phi_val = phi_func(u, yd, alpha, beta, u_a, u_b, G_u)
