...
TRAIN_PATH = './data/semilinparab/sp_cts_train.npz'
TEST_PATH = './data/semilinparab/sp_cts_test.npz'
...
for ep in range(epochs):
    ...
    for u, s in train_loader:
        optimizer.zero_grad()
        out = hc * model(u).view(batch_size, S, S, T)
...
with torch.no_grad():
    for u, s in test_loader:
        out = hc_single.to(device) * model(u).view(S, S, T)
        pred[index] = out
...
if save_pred == 'npy':
    np.save('sp_pred_cts_fno3d_hc.npy', pred.cpu().numpy())
elif save_pred == 'mat':
    sio.savemat('sp_pred_cts_fno3d_hc.mat', mdict={'pred': pred.cpu().numpy()})
